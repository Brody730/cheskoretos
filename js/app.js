    ...
    async function manejarEscaneoQR(targetUserId) {
        ...
        try {
            ...
            if (datos === undefined) {
                ...
                var _e = DataStore.obtenerUltimoErrorPerfil ? DataStore.obtenerUltimoErrorPerfil() : null;   /* [DIAG] */
                var _diag = _e ? (' [code=' + (_e.code || '?') + ' status=' + (_e.status || '?') + ' msg=' + (_e.message || _e.hint || '?') + ']') : '';
                mostrarAlerta('❌', 'Error de Conexión', 'No se pudo verificar el código QR.' + _diag + ' id=' + targetUserId, 'error');
                return;
            }
